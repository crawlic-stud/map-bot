...
location_kb = types.ReplyKeyboardMarkup(
    keyboard=[[types.KeyboardButton(text=SAVE_TEXT, request_location=True)]],
...
async def save_location(m: types.Message, state: FSMContext):
    ...
    address = await get_address(lat=loc.latitude, lon=loc.longitude)
...
async def save_name(m: types.Message, state: FSMContext):
    ...
    res = await locations_db.update_one({"_id": loc_id}, {"$set": {"name": m.text}})
